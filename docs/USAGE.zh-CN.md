...
| `DUMP_PEAK_STEP_MB` | 给正值并与 `ALLOC_HOOK_PEAK_SAMPLE_MS` 一起使用时选择峰值追踪；单独设置不起作用，`0`（默认）表示关闭追踪。表示重建峰值快照所需增长量的上限；小峰值使用 25% 增长量，下限 64 KB。首次越线模式下不生效。 |
| `ALLOC_HOOK_PEAK_SAMPLE_MS` | 采样实测 `VmRSS + DMA + GPU` 总量的毫秒间隔，所有模式的峰值判据都是它。只设它即选择只观测探测模式（见 3.1）：测量并打印，不做跟踪。默认取宿主框架公布的间隔，配置了报告而框架未公布时为 50ms。`0` 表示强制不起采样线程。 |
...
设置了哪些变量决定这次运行是否跟踪分配。只设 `ALLOC_HOOK_PEAK_SAMPLE_MS` 表示测量
实测占用并在退出时打印日志，不跟踪任何分配（见 3.1）。再加 `DUMP_PEAK_STEP_MB` 选择
峰值追踪：快照按步长重建，最终落在运行期最大值上，且不需要事先知道这个最大值。
`DUMP_PEAK_VALUE_MB` 选择首次越线：整个运行只抓一张快照，即判据首次越过下限的那一刻，
之后不再有分配线程被快照阻塞。两种报告模式都会在退出时写到 dump 前缀并自动创建目录；
探测模式不写文件。
...
export DUMP_PEAK_STEP_MB=1           # 最小的有效步长；0 表示关闭追踪
...
`0` 表示关闭对应功能，而不是"不设上限地打开"：`DUMP_PEAK_STEP_MB=0` 表示不做追踪，
`DUMP_PEAK_VALUE_MB=0` 表示不要首次越线快照。给一个较小的正步长即可让快照贴近最大值
（小峰值时代码实际按 25% 增长量处理）。只有确实需要 1 KB 以下
分配的堆栈时才设置 `BACKTRACE_MIN_SIZE=0`。要保持精确 host 归因且不抑制满足尺寸条件
的堆栈，两个字节采样变量都应保持未设置（实际值为 `1`）。

### 3.1 只观测探测模式

只设置采样间隔、不设置其他变量，即可在不跟踪任何分配的前提下测量占用：

```sh
LD_PRELOAD="$PWD/out/lib/liballoc_hook.so" ALLOC_HOOK_PEAK_SAMPLE_MS=5 ./your_program
```

退出时向 stderr 打印摘要，形状和黄色标识都与宿主框架为同样三项打印的那段一致（版式和
各行读法见 README）；颜色只在 stderr 是终端时才输出。不会生成报告文件，因为没有任何被
跟踪的数据可写。

符号插入仍然发生——加载器已经完成绑定——但每次被插入的调用都直接转发给 libc，所以这次
运行付的是采样线程加每次调用一次 relaxed 读，而不是每次分配一个被跟踪的指针加一份抓到
的栈：实测每对 `malloc`/`free` 68 ns，不预加载时 60 ns，而没有尺寸过滤的跟踪模式是
2762 ns。

用它先确认一个进程有没有内存问题、有多大。它不抓栈，所以无法把总量归因到调用点；也没有
存活分配表，因此 `checkpoint()` 把实测数据写到指定路径而不是堆报告，checkpoint 信号被
忽略而不是放任它打死进程；`fork` 子进程不打印，通过 `_exit()` 或致命信号离开的进程什么
都不打印。当问题变成"内存花在哪"时，再加 `DUMP_PEAK_STEP_MB` 或 `DUMP_PEAK_VALUE_MB`。
