...
* how to build
  ...
  * ./build_ohos.sh [arm64-v8a|armeabi-v7a|x86_64]
    * set `OHOS_NDK_ROOT` to an OHOS `native` NDK directory, or set `OHOS_NDK_URL` to an NDK zip URL and the script downloads it into `.deps/ohos_ndk/native`
    * output: `out/lib/liballoc_hook.so`

* support matrix / 支持状态

  下表为各 API 的内存分配是否被统计（对应符号是否被 hook / 导出）。导出集合随系统与构建配置而定；同一系统下不同架构的导出符号集合一致。

| 能力 capability | Android | OHOS · 默认 | OHOS · `OHOS_ENABLE_MMAP_HOOK=ON` |
| --- | :---: | :---: | :---: |
| 架构 arch | arm64-v8a / armeabi-v7a | arm64-v8a / armeabi-v7a / x86_64 | arm64-v8a / armeabi-v7a / x86_64 |
| malloc 家族<br>`malloc` `free` `calloc` `realloc` `aligned_alloc` `memalign` `posix_memalign` | ✅ | ✅ | ✅ |
| operator new / delete<br>`_Znwm` 等 mangled 符号 | ✅ ¹ | ✅ | ✅ |
| mmap / munmap / mmap64 | ✅ | ❌ ² | ✅ |
| ioctl / close | ✅ | ✅ | ✅ |
| checkpoint | ✅ | ✅ | ✅ |

  - ¹ Android 未直接导出 `operator new/delete`；libc++ 的 `new`/`delete` 经被 hook 的 `malloc`/`free` 间接统计。OHOS 静态链接 libc++，`new` 不保证走到被 hook 的 `malloc`，故显式导出 `_Znwm` 等 mangled 符号。
  - ² OHOS 默认不导出 `mmap`/`munmap`，避免在 loader / OpenCL / vendor runtime 启动阶段 interpose `mmap` 造成扰动或死锁；需要时用 `OHOS_ENABLE_MMAP_HOOK=ON ./build_ohos.sh <arch>` 构建（建议仅用于小型复现程序）。`mmap64` 在 glibc/Android 为独立符号，musl(OHOS) 将其别名为 `mmap`。
