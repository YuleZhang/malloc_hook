...
NDK_URL="${OHOS_NDK_URL:-http://release_backup.hpc.mbg.jiigan.com:9999/download/treasurebox/ftp_data/ohos_ndk/native-linux-x64-5.0.0.12-Canary1.zip}"
...
resolve_ndk_root() {
    ...
    if [[ ! -f "${AUTO_NDK_DIR}/build/cmake/ohos.toolchain.cmake" ]]; then
        mkdir -p "${DEPS_DIR}"
        local zip_path="${DEPS_DIR}/native-linux-x64-5.0.0.12-Canary1.zip"
        if [[ ! -f "${zip_path}" ]]; then
            echo "Downloading OHOS NDK to ${zip_path}"
